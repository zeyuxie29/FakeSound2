...
class BaseDetectionModel(nn.Module):
    def __init__(self, embed_dim=128, in_planes=1024, 
        ...
        self.bce_classifier = nn.Sequential(
            nn.ReLU(),
            # BiLSTM
        ...
        if label_info:
            self.multitask_classifier = nn.ModuleDict({
                label_type: nn.Linear(embed_dim*2, len(label_info[label_type]))
    ...
    def forward(self, waveform):
        ...
        output = {"pred_BCE" : self.bce_classifier(x).squeeze(-1)}
        
        if self.label_info:
            for label_type in self.label_info:
                output[f"pred_{label_type}"] = self.multitask_classifier[label_type](torch.mean(x, dim=1))

        return output

class WavLM_Detection(BaseDetectionModel):
    def __init__(self, embed_dim=128, in_planes=1024, label_info=None):
        super().__init__(embed_dim=128, in_planes=in_planes, label_info=label_info)


        from WavLM import WavLM, WavLMConfig

        # load the pre-trained checkpoints
        checkpoint = torch.load('~/.cache/deepfake/WavLM-Large.pt')
...
class EAT_Detection(BaseDetectionModel):
    def __init__(self, embed_dim=128, in_planes=768, label_info=None):
        super().__init__(embed_dim=128, in_planes=in_planes, label_info=label_info)

        import fairseq

        model_dir = 'models/EAT/EAT'
        checkpoint_dir = '~/.cache/deepfake/EAT_base_epoch20.pt'
...
class EAT_Trivial_Detection(EAT_Detection):
    def __init__(self, embed_dim=128, in_planes=768, label_info=None):
        super().__init__(embed_dim=128, in_planes=in_planes, label_info=label_info)
        self.backbone = nn.Sequential(
            nn.Linear(in_planes, embed_dim*2),
            BatchNorm1d(input_size=embed_dim*2),
        )